...
;	メニューボタン非表示
	...
	TG.config.autoRecordLabel = "true"; // Enable label passage recording
	...
	if(tf.user_setting != 'default'){
		TG.config.alreadyReadTextColor = 'default'; // Temporarily not changing the text color of already-read text
	}
...
;	画面右上の「Back」ボタン
	[button fix="true" graphic="&tf.img_path + 'c_btn_back.png'" enterimg="&tf.img_path + 'c_btn_back2.png'" target="*backtitle" x="1160" y="20" name="rightClickButton"]

;　右クリックでジャンプ
[back_set target="*backtitle"]
...
[button role="fullscreen" graphic="./data/image/button/fullsc01.png" enterimg="./data/image/button/fullsc02.png" x="920" y="455"]
...
[cm]
	...
	[iscript]
	TG.config.alreadyReadTextColor = tf.user_setting; // Restore the text color of already-read text
	[endscript]
